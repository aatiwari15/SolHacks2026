...
function PlaceholderChat({ agent, label }: { agent: string; label: string }) {
  ...
  return (
    <div className="flex flex-1 flex-col items-center justify-center gap-4 text-center px-6">
      <div
        className={`flex h-14 w-14 items-center justify-center rounded-2xl text-lg font-bold text-white ${isTeal ? "bg-teal-500" : "bg-lime-500"}`}
      >
        {isTeal ? "Ha" : "Si"}
      </div>
      <div>
        <p
          className={`text-base font-semibold ${isTeal ? "text-teal-500" : "text-lime-500"}`}
        >
          {label}
        </p>
        <p className="text-sm text-fg-muted mt-1">
          Coming soon — this flow is being built.
        </p>
      </div>
    </div>
  );
